...
| `formatter`     | `string`  | A custom formatter for the column values. Treat it like a [Javascript template literal string](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Template_literals) with access to the variables `$cell` and `$row`<br> Examples: <ul><li>`"#${$row.someOtherColumn}) <strong>${$cell}</strong>"`</li></ul>                                                                                                |
