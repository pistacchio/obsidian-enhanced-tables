...
class TableDocument {
  ...
  private static documentToTable(fileContent: string): [string, string[], string] | null {
    ...
    for (let lineNo = 0; lineNo < lines.length; lineNo++) {
      ...
      if (startingLine !== null && lineNo === startingLine + 1 && !line.match(SUB_HEADER_LINE_REGEX)) {
        startingLine = null;
        continue;
      }
...
export class TableManager {
  public insertLine(fileContent: string, lineNo: number, values: string[]): string {
  ...
  public modifyLine(fileContent: string, lineNo: number, values: string[]): string {
    const tableDocument = new TableDocument(fileContent);
  ...
  public async insertLineToFile(file: TFile, vault: Vault, lineNo: number, values: LineValues): Promise<void> {
    const fileContent = await vault.read(file);
    const modifiedFileContent = this.insertLine(fileContent, lineNo, values);
    await vault.modify(file, modifiedFileContent);
  }

  public async modifyLineInFile(file: TFile, vault: Vault, lineNo: number, values: LineValues): Promise<void> {
    const fileContent = await vault.read(file);
    const modifiedFileContent = this.modifyLine(fileContent, lineNo, values);
    await vault.modify(file, modifiedFileContent);
  }

  public async modifyHeaderInFile(file: TFile, vault: Vault, values: LineValues) : Promise<void>{
    const fileContent = await vault.read(file);
    const modifiedFileContent = this.modifyHeader(fileContent, values);
    await vault.modify(file, modifiedFileContent);
  }

  public async removeLineFromFile(file: TFile, vault: Vault, lineNo: number) : Promise<void>{
    const fileContent = await vault.read(file);
    const modifiedFileContent = this.removeLine(fileContent, lineNo);
    await vault.modify(file, modifiedFileContent);
  }

  public async readLineFromFile(file: TFile, vault: Vault, lineNo: number): Promise<LineValues | null> {
    const fileContent = await vault.read(file);
    return this.readLine(fileContent, lineNo);
  }

  public async readTableLinesFromFile(file: TFile, vault: Vault): Promise<LineValues[] | null> {
    const fileContent = await vault.read(file);
